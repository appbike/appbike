...
@property (nonatomic, strong) NSMutableArray *arrLatLong;
...
- (void)setChartData:(NSArray *)chartData
{
    ...
    self.arrLatLong = [[NSMutableArray alloc] init];
    ...
    NSLog(@"Maximum Elevator is : %f",_max);
    // No data
    if(isnan(_max)) {
        _max = 1;
    }
    ...
    if(_labelForIndex) {
        ...
        for(int i=0;i<_gridStep + 1;i++) {
            ...
            int iKM = appDelegate().iRemainingKM / (4 - i);
            //NSString* text = [NSString stringWithFormat:@"%d",i+1];
            NSString* text = [NSString stringWithFormat:@"%d",iKM];
            if(i==0)
                text = @"KM";
            CGPoint p = CGPointMake(_margin + i * (_axisWidth / _gridStep) * scale, _axisHeight + _margin);
            
            UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - 4.0f, p.y + 2, self.frame.size.width + 5, 14)];
            label.text = text;
...
- (int)getChartIndexFromData:(NSString *)value withLocation:(CLLocation *)currentLocation
{
    int index = 0;
    
    //Both conditions are not satisfy then we move ahead with distance filter
    CLLocationDistance totaldistance = [appDelegate().toLocation distanceFromLocation:appDelegate().fromLocation];
    
    //If there are less then 5KM total distance then we apply devider by 10 logic becase same elevation is repeated more then once
   // if(totaldistance <= 5000)
    {
        CLLocationDistance distanceCovered = [currentLocation distanceFromLocation:appDelegate().fromLocation];
        double incrementValue = totaldistance / 10.0f;
        int myIndex = 0;
        for(double iCounter=0; iCounter <= totaldistance; iCounter+=incrementValue)
        {
            NSLog(@"checking distance cover : %f and counter : %f",distanceCovered,iCounter);
            if(distanceCovered >= iCounter)
            {
                myIndex++;
                //return myIndex;
            }
        }
        NSLog(@"Now My Index is : %d",myIndex);
        return myIndex;
    }
    for(int i=0;i<_data.count;i++)
    {
        ...
        if([text isEqualToString:value])
        {
            ...
            if(myindex == _data.count)
                myindex = myindex-1;
            
            NSDictionary *matchLocation = [self.arrLatLong objectAtIndex:myindex];
            
//            location =     {
//                lat = "36.455556";
//                lng = "-116.866667"; int elevation = [[dictionary valueForKey:@"elevation"] intValue];
//            };

            
            double latitude = [[[matchLocation valueForKey:@"location"] valueForKey:@"lat"] doubleValue];
            double longitude = [[[matchLocation valueForKey:@"location"] valueForKey:@"lng"] doubleValue];
            ...
            NSLog(@"Chart Match but distance is : %f",distance);
            if(distance <= 100 && distance>0) //If dist
            {
                return i+1;
            }
            else if(distance < 0 )
            {
                //Means we are moving opposit direct
                return i-1;
            }
        }
        int iValue = [value intValue];
        if(iValue <= [number intValue])
        {
            index = i;
...
- (void)strokeChart
{
    ...
    for(int i=1;i<_data.count;i++) {
        ...
        NSString* text = [NSString stringWithFormat:@"%@",number];
        CGRect rect = CGRectMake(_margin, p1.y + 2, self.frame.size.width - _margin * 2 - 4.0f, 14);
        ...
        UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p1.x - width - 6, p1.y + 2, width + 2, 14)];
        label.text = text;
        label.font = [UIFont systemFontOfSize:12.0f];
        label.textColor = [UIColor grayColor];
        label.textAlignment = NSTextAlignmentCenter;
    ...
    for(int i=0;i<_data.count;i++)
    {
        ...
        CGPoint p1 = CGPointMake(_margin + i * (_axisWidth / (_data.count)), _axisHeight + _margin - [number floatValue] * scale);
        
        NSString* text = [NSString stringWithFormat:@"%@",number];
        CGRect rect = CGRectMake(_margin, p1.y + 2, 320 - _margin * 2 - 4.0f, 14);
        ...
        //        UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p1.x - width - 6, p1.y - 2, width + 2, 14)];
        UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p1.x , p1.y - 12, width + 2, 14)];
        //UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p1.x , 0, width + 2, 14)];
        label.text = text;
        label.font = [UIFont systemFontOfSize:12.0f];
        label.textColor = [UIColor grayColor];
        label.textAlignment = NSTextAlignmentCenter;
        ...
        NSDictionary *dictionary1 = [[NSDictionary alloc ] initWithObjectsAndKeys:[NSString stringWithFormat:@"%f",p1.x],@"x",[NSString stringWithFormat:@"%f",p1.y-12],@"y", nil];
        
        [appDelegate().arrayCoords addObject:dictionary1];
